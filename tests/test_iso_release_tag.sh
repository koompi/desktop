...
publish() {
    ( cd "$tmp/out" && PATH="$tmp/bin:$PATH" GITHUB_REPOSITORY=koompi/koompi-desktop GITHUB_SHA="$1" bash "$tmp/publish.sh" )
}
...
grep -q -- '--repo koompi/koompi-desktop' "$tmp/gh.log" || fail "the repo flag was lost"
